...
export default function CreateServerModal({
  ...
  return (
    ...
    >
      <div className="surface-panel relative flex h-[calc(100dvh-1.5rem)] max-h-[calc(100dvh-1.5rem)] w-full max-w-6xl min-h-0 flex-col overflow-hidden rounded-[2rem] sm:h-[min(56rem,calc(100dvh-3rem))] sm:max-h-[min(56rem,calc(100dvh-3rem))]">
        ...
        <div className="shrink-0 border-b border-border/70 px-5 py-5 sm:px-6">
          <div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
            ...
            <button
              type="button"
              onClick={onClose}
              className="inline-flex h-10 w-10 items-center justify-center self-end rounded-full border border-border/70 bg-background/70 text-muted-foreground transition hover:border-primary/30 hover:text-foreground sm:self-auto"
            >
              <X className="h-4 w-4" />
            </button>
          </div>
        ...
        <div className="flex-1 min-h-0 overflow-y-auto overscroll-contain xl:overflow-hidden">
          <div className="flex min-h-full flex-col xl:grid xl:h-full xl:min-h-0 xl:grid-cols-[minmax(0,1fr)_22rem]">
            <main className="px-5 py-5 pb-8 sm:px-6 xl:min-h-0 xl:overflow-y-auto xl:overscroll-contain">
              {errors.submit && (
          ...
          <div className="flex flex-col gap-3 lg:flex-row lg:items-center lg:justify-between">
            <div className="min-w-0">
              ...
              <p className="mt-1 truncate text-sm text-muted-foreground">
                {name || "VM mới"} • {flavor || "Chưa chọn flavor"} •{" "}
                {osName || "Chưa chọn image"}
              </p>
            </div>
...
function EnvironmentCard({
  ...
  return (
    <button
      type="button"
      onClick={onToggle}
      className={`h-full rounded-[1.4rem] border p-4 text-left transition ${
        selected
          ? "border-primary/40 bg-primary/10"
          : "border-border/70 bg-background/65 hover:border-primary/25"
      }`}
